  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const datosVehicle = {
        ...
        year_manufacture_vehicle: values.year_manufacture_vehicle,
        vehicle_color: values.vehicle_color,
        vehicle_plate: values.vehicle_plate,
        long_useful_space: values.long_useful_space,
        width_useful_space: values.width_useful_space,
        height_useful_space: values.height_useful_space,
        type_file_soat: 'pdf_jpg',
        policy_soat_number: values.policy_soat_number,
        soat_expiration_date: values.soat_expiration_date,
        partida_registral: values.partida_registral,
      ...
      console.log({
        taxista: { ...registroData.datosVehiculo, ...datosVehicle },
        vehicle: { ...registroData.datosDriver, ...datosDriver }
      })

      /*  await Promise.all([
        fetch(API_URL + '/api/user/delivery/vehicle', {
          method: 'POST',
          body: { ...registroData.datosVehiculo, ...datosVehicle },
          headers: {
        ...
        fetch(API_URL + '/api/user/delivery/information', {
          method: 'POST',
          body: { ...registroData.datosDriver, ...datosDriver },
          headers: {
            Authorization: `Bearer ${token}`,
              'Content-Type': 'multipart/form-data',
          }
        })
      ]).then(res => res.map(res => res.status === 'fulfilled'))
