            ...
            <View style={styles.movimientoItem}>
                ...
                <View style={styles.movimientoMonto}>
                    ...
                    <View style={styles.montoContainer}>
                        ...
                        />
                        <Text style={[styles.montoTexto, item.monto > 0 ? styles.montoPositivo : styles.montoNegativo]}>
                            {item.tipo === 'TRANS' ? '' : (item.monto > 0 ? '+' : '-')}S/ {Math.abs(item.monto).toFixed(2)}
                        </Text>
                    </View>
                    <Text style={styles.texto}>Trans N° {item.transaccion}</Text>
                    {item.estado === 'realizado' && <Ionicons name="checkmark-circle" size={24} color="green" />}
                    {item.estado === 'pendiente' && <MaterialIcons name="pending-actions" size={24} color="orange" />}
                    {item.estado === 'error' && <Ionicons name="close-circle" size={24} color="red" />}
    ...
    return (
        <Container>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                <Button title="Retirar" primary onPress={()=>navigation.navigate("PorRedRetirar")} />
                <Button title="Transferir" primary onPress={()=>navigation.navigate("PorRedTransferir")} />
            </View>
        </Container>
    );
};
